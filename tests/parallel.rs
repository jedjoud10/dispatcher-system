...
fn simpler() {
    ...
    let sorted = registry.sort().unwrap();
    assert_eq!(sorted.group(0), Some(&vec![StageId::of(&system_a), StageId::of(&system_b)]));
}
...
fn all() {
    ...
    let sorted = registry.sort().unwrap();
    assert_eq!(sorted.group(0).unwrap().len(), 5);
}
...
fn balancing() {
    ...
    let mut sorted = registry.sort().unwrap();
    sorted.balance(Some(2));
    assert_eq!(sorted.group(0).unwrap().len(), 2);
    assert_eq!(sorted.group(1).unwrap().len(), 2);
    assert_eq!(sorted.group(2).unwrap().len(), 1);
}
